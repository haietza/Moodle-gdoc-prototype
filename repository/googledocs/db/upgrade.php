...
function xmldb_repository_googledocs_upgrade($oldversion) {
    ...
    if ($oldversion < 2016083102) {

        // Define table google_files_reference to be created.
        $table = new xmldb_table('google_files_reference');
        ...
        // Conditionally launch create table for google_files_reference.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }
        ...
        upgrade_plugin_savepoint(true, 2016083102, 'repository', 'googledocs');
    }
